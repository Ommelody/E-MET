                    ...
                    <tr key={r.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex justify-center gap-1">
                          {r.RequisitionPDFLink && <button onClick={() => window.open(r.RequisitionPDFLink, "_blank")} title="เอกสารใบเบิก" className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border-none bg-transparent text-slate-400 hover:bg-indigo-50 hover:text-indigo-600"><FileText className="h-4 w-4" /></button>}
                          {r.GoodsIssuePDFLinks?.map((g: any, i: number) => (
                            <button key={i} onClick={() => window.open(g.url, "_blank")} title={`เอกสาร/พิมพ์ใบจ่าย ${g.id}`} className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg border-none bg-transparent text-slate-400 hover:bg-emerald-50 hover:text-emerald-600"><Printer className="h-4 w-4" /></button>
                          ))}
                        </div>
                      </td>
        ...
        {detailLoading || detail?.loading ? <Spinner /> : detail && (
          <div>
            ...
            <div className="mt-4 flex justify-end gap-2">
              {detail.requisition.RequisitionPDFLink && <Button variant="outline" onClick={() => window.open(detail.requisition.RequisitionPDFLink, "_blank")}><FileText className="h-4 w-4" />ใบเบิก</Button>}
              {detail.requisition.GoodsIssuePDFLinks?.map((g: any, i: number) => <Button key={i} variant="outline" onClick={() => window.open(g.url, "_blank")}><FileDown className="h-4 w-4" />ใบจ่าย {i + 1}</Button>)}
            </div>
          </div>
        )}
      </Modal>
