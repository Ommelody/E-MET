export function resolveApiUrl(path: string): string {
  ...
  // Default: same-origin. When frontend and backend are deployed together
  // (Render / Railway / Cloud Run / local dev), API calls stay relative ("").
  // Only when hosted as a static-only site (e.g. GitHub Pages) do we need a
  // separate backend URL — set it via the "ตั้งค่าจุดเชื่อมต่อเซิร์ฟเวอร์ API"
  // button on the login screen (stored in localStorage as backend_api_url).
  const apiBase = localStorage.getItem("backend_api_url") || "";
