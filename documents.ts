...
function baseStyle() {
  return `@import url('https://fonts.googleapis.com/css2?family=Sarabun:wght@400;600;700&display=swap');
    *{box-sizing:border-box}
    ...
    .print-bar{position:fixed;top:0;left:0;right:0;display:flex;gap:8px;justify-content:space-between;align-items:center;z-index:99;padding:10px 12px;background:rgba(255,255,255,.95);border-bottom:1px solid #e2e8f0;box-shadow:0 2px 8px rgba(0,0,0,.08)}
    .print-bar button{font-family:'Sarabun',sans-serif;font-size:12pt;font-weight:600;border:none;border-radius:10px;padding:11px 18px;cursor:pointer}
    .btn-print{background:#4f46e5;color:#fff}
    .btn-close{background:#e2e8f0;color:#1e293b}
    body{margin-top:60px}
    @media print{.print-bar{display:none}body{margin:0!important}}`;
}

const PRINT_BAR = `<div class="print-bar">
  <button class="btn-close" onclick="if(window.opener){window.close();}else{history.length>1?history.back():window.close();}">\u2190 ปิดเอกสาร</button>
